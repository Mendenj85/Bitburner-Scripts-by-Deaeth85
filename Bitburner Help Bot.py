...
async def bn4(ctx):
    ...
    embed.add_field(name="BN 4.2", value="4x ram cost multiplier", inline=False)
    embed.add_field(name="BN 4.3", value="1x ram cost multiplier", inline=False)
    await ctx.send(embed=embed)
...
async def favor(ctx):
    ...
    embed.add_field(name="Augmentation Install", value="You gain favor based on earned rep after an aug install", inline=False)
    embed.add_field(name="How much on install?", value="You can mouse over the reputation on main page of the faction to see how much favor you will earn on aug install", inline=False)
    embed.add_field(name="Getting 150 Favor/Donation Ability", value="If you do it in one shot, it takes roughly 462.5k rep to get the 150 favor needed for donations", inline=False)
    embed.add_field(name="Getting Donation Status Faster", value="This is easier to achieve by earning roughly 70k - 100k rep, then doing an aug install, as the next run you will earn more favor, and faster", inline=False)
    await ctx.send(embed=embed)
...
async def format(ctx):
    ...
    embed.add_field(name="Turns into:", value='```js\nwhile (true) {\n ns.exec(my_script.js, "home");\n await ns.sleep(100)\n}\n```', inline=False)
    embed.set_footer(text="NOTE: There are tabs in the top part, they just don't show up on embed formatting, but you see them in the code block")
    await ctx.send(embed=embed)
...
async def rep(ctx):
    ...
    embed.add_field(name="Augmentations", value="Auguments with reputation bonus (includes Neuroflux Governor)", inline=False)
    embed.add_field(name="Favor", value="1 favor = 1% faster rep gain", inline=False)
    embed.add_field(name="Donations", value="unlocked at 150 favor, 1 rep per $1m donated, before bonuses", inline=False)
    embed.add_field(name="Infiltrations", value="Successful infiltrations can earn rep", inline=False)
    embed.add_field(name="Coding Contracts", value="give rep to a single joined faction, or spread across all joined factions", inline=False)
    embed.add_field(name="Endgame", value="||high Intelligence has an effect on reputation gain||", inline=False)
    embed.add_field(name="Endgame", value="||Corps can directly add rep with Corp funds||", inline=False)
    await ctx.send(embed=embed)
...
async def singularity(ctx):
    embed=discord.Embed(title="Singularity API", url="https://i.ibb.co/LSbWqj0/Bitburner-Logo.png", description="Click this link - <https://github.com/danielyxie/bitburner/blob/master/markdown/bitburner.singularity.md>", color=0x00ff00)
    embed.set_author(name="Bitburner Help Bot ", icon_url="https://i.ibb.co/LSbWqj0/Bitburner-Logo.png")
    embed.set_thumbnail(url="https://i.ibb.co/LSbWqj0/Bitburner-Logo.png")
    await ctx.send(embed=embed)
...
async def startgang(ctx):
    ...
    embed.add_field(name="Outside of BN 2", value="You need -54k Karma to start a gang", inline=False)
    embed.add_field(name="How to get the Karma", value="This equates to 15 hours of 100% success rate homicide", inline=False)
    embed.add_field(name="How to get the Karma faster", value="Sleeves can help reduce this time drastically", inline=True)
    await ctx.send(embed=embed)
